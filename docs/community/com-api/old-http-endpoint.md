...
* [Obtain an auth token](/docs/community/com-auth/com-auth-token).
...
The query is executed for all [shards](/docs/concepts/glossary#s) by default. To use specified shards only, set the `shards` query argument to a comma-separated list of slice indices.
...
The problem is that users of FeatureBase (such as [ingesters](/docs/community/com-ingest/com-ingest-manage))
